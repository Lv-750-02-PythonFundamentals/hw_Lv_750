...
# extracting game sprites form the resource.png image.
...
# initializing variables which store current game status
...
while not crashed:
    ...
    # generate and animate random obstacles
    ...
    if height< 100:
        start=True
    if start:
        # check if obstacle is hit by player
        ...
        # determening which dinosaur sprite to use depends on whether it hit obstacle
        if not lock:
            bg = (bg[0]-speed, bg[1])
            if bg[0]<=-(600):
                lock = 1
